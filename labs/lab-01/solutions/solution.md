...
- Run:
  - `docker version`
  - `docker info`
- If `docker info` fails, the daemon isn’t reachable:
  - Docker Desktop may not be running
  - On Linux, the Docker service may be stopped
...
- Confirm port mapping:
  - `docker port lab01-nginx`
- If port 8080 is already taken:
  - Recreate with `-p 8081:80` and visit `http://localhost:8081`
...
- Refresh the page a few times, then re-run:
  - `docker logs lab01-nginx --tail 50`

## If `docker exec -it ... sh` fails

- Some images don’t include `sh` or `bash`
- Nginx does include `sh`, but if it fails, try:
  - `docker exec -it lab01-nginx /bin/sh`

# Lab 01 — Solution (reference)

> Use this only after attempting the lab with `hints.md`.
...
If the port was different (e.g., 8081), adjust the URL accordingly.
